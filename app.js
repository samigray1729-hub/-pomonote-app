(() => {
  ...
  function renderSettings() {
    ...
    if (!("Notification" in window)) {
      ...
      $("#notificationStatus").textContent = "このブラウザーは通知に対応していません。";
    } else {
      const labels = { granted: "通知は許可されています。", denied: "通知が拒否されています。ブラウザー設定から変更できます。", default: "通知はまだ許可されていません。" };
      $("#notificationStatus").textContent = labels[Notification.permission];
      $("#requestNotifications").textContent = Notification.permission === "granted" ? "通知を確認" : "通知を許可";
    }
  }
  ...
  async function sendNotification(title, body) {
    ...
    try {
      if ("serviceWorker" in navigator) {
        ...
        await registration.showNotification(title, { body, icon: "./icons/icon.svg", badge: "./icons/icon.svg", tag: "pomonote-timer", renotify: true });
      } else {
  ...
  async function requestNotificationPermission() {
    ...
    try {
      ...
      renderSettings();
      if (Notification.permission === "granted") showToast("通知を許可しました。");
    } catch (error) {
